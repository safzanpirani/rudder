...
bin_dir=${RUDDER_BIN_DIR:-"$HOME/.local/bin"}
data_home=${XDG_DATA_HOME:-"$HOME/.local/share"}
install_dir="$data_home/rudder"

command -v bun >/dev/null 2>&1 || {
  printf '%s\n' "rudder install: Bun 1.4 or newer is required" >&2
  exit 1
}
...
test -f "$install_dir/tui/index.ts" || {
  printf '%s\n' "rudder install: installed TUI entry is missing" >&2
  exit 1
}
...
source_hash=$(shasum -a 256 "$repo_dir/rudder" | awk '{print $1}')
installed_hash=$(shasum -a 256 "$bin_dir/rudder" | awk '{print $1}')
test "$source_hash" = "$installed_hash" || {
  printf '%s\n' "rudder install: installed binary verification failed" >&2
  exit 1
}

printf '%s\n' "installed $bin_dir/rudder"
